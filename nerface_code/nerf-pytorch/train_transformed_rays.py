...
def main():
    ...
    if cfg.dataset.type.lower() == "blender":
        images, poses, render_poses, hwf, i_split, expressions, ids, bboxs = load_flame_data(
            cfg.dataset.basedir,
    ...
    if fixed_background: # load GT background
        ...
        background = Image.open(os.path.join(cfg.dataset.basedir,'bg.png'))
    ...
    for i in trange(start_iter, cfg.experiment.train_iters):
        ...
        background_ray_values = background[select_inds[:, 0], select_inds[:, 1], :] if fixed_background else None
        #if i<10000:
        #   background_ray_values = None
        #background_ray_values = None
        then = time.time()
        ...
        loss_total = loss + (latent_code_loss*10 if regularize_latent_codes else 0.0)
        #loss.backward()
        loss_total.backward()
        #psnr = mse2psnr(loss.item())
        optimizer.step()
